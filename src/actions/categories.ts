...
export const imageUploadHandler = async (formData: FormData) => {
  ...
  if (!(fileEntry instanceof File)) throw new Error("Invalid file entry");
  ...
  try {
    const { data, error } = await supabase.storage
      .from("app-image")
      .upload(fileName, fileEntry, { cacheControl: "3600", upsert: false });
    ...
    const {
      data: { publicUrl },
    } = supabase.storage.from("app-image").getPublicUrl(data.path);
